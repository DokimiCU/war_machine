...
minetest.register_item(":", {
	...
	tool_capabilities = {
		...
		groupcaps = {
			choppy = {times={[1]=minimal.hand_chop*3, [2]=minimal.hand_chop*2, [3]=minimal.hand_chop}, uses=0, maxlevel=minimal.hand_max_lvl},
			crumbly = {times={[3]=minimal.hand_crum}, uses=0, maxlevel=minimal.hand_max_lvl},
			cracky = {times={[3]=minimal.hand_crac}, uses=0, maxlevel=minimal.hand_max_lvl},
			snappy = {times={[1]=minimal.hand_snap*3, [2]=minimal.hand_snap*2, [3]=minimal.hand_snap}, uses=0, maxlevel=minimal.hand_max_lvl},
...
minetest.register_on_joinplayer(function(player)
	minetest.get_inventory({type="player", name=player:get_player_name()}):set_size("main", 32)
end
)
